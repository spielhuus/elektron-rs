...
use crate::plot::{Theme, Plotter, PlotOptions, PlotSelector};
use crate::sexp::SchemaElement;
use pyo3::prelude::*;
...
use crate::draw::{At, Attribute, Direction, Dot, DotPosition, Draw, Drawer, Label, Nc, Symbol, To};
...
macro_rules! dot {
    ($attributes:expr, $element:expr) => {
        if let Some(dot) = $element.dot {
            let dot: Vec<DotPosition> = dot.iter().map(|d| {
                d.as_str().into()
            }).collect();
...
impl PyDraw {
    ...
    fn add(&mut self, item: &'_ PyAny) -> PyResult<()> {
        ...
        if let Ok(feedback) = feedback {
            if let Some((reference, pin)) = feedback.atref {
                //draw the first line
                self.draw.draw(&At::Pin(reference.to_string(), pin.to_string()))?;
                let mut to = To::new();
                ...
                if let Some((to_reference, to_pin)) = feedback.toref {
                    let mut to = To::new();
                    to.attributes.push(Attribute::Tox(At::Pin(to_reference.to_string(), to_pin.to_string())));
                    self.draw.draw(&to)?;

                    //and close it
                    let mut to = To::new();
                    to.attributes.push(Attribute::Toy(At::Pin(to_reference.to_string(), to_pin.to_string())));
                    self.draw.draw(&to)?;

                    if let Some(dots) = feedback.dot {
                        let dot: Vec<DotPosition> = dots.iter().map(|d| d.as_str().into()).collect();
                        for d in dot {
                            match d {
                                DotPosition::Start => {
                                    let d = Dot::new();
                                ...
                                DotPosition::End => {
                                    let d = Dot::new();
                                    self.draw.draw(&At::Pin(to_reference.to_string(), to_pin.to_string()))?;
                                    self.draw.draw(&d)?;
                                }
                            }
                        }
        ...
        if let Ok(line) = line {
            ...
            attributes!(to.attributes, line, length, direction, tox, toy, toxref, toyref, toxdot, toydot, dot);
            let wire = self.draw.draw(&to)?;
            if let Some(SchemaElement::Wire(wire)) = wire {
                item.call_method1("set_points", ((wire.pts[[0, 0]], wire.pts[[0, 1]]), (wire.pts[[1, 0]], wire.pts[[1, 1]])))?;
            } else {
        ...
        if let Ok(dot) = dot {
            ...
            if let Some(SchemaElement::Junction(junction)) = junction {
                if dot.pushed {
                    self.positions.push((junction.at[0], junction.at[1]));
                }
            }
            return Ok(());
        }
        ...
        if let Ok(element) = element {
            ...
            attributes!(symbol.attributes, element, anchor, mirror, length, rotate, tox, toy, toxref , toyref, toxdot, toydot, property);
    ...
    pub fn property(&mut self, regex: String, key: String, value: String, id: u32) -> Result<(), Error> {
        self.draw.property(regex, key, value, id)
    }
    ...
    pub fn plot(&mut self, kwargs: Option<&PyDict>) -> Result<Option<Vec<Vec<u8>>>, Error> {
        ...
        if let Some(kwargs) = kwargs {
            if let Some(item) = kwargs.get_item("filename") {
                let item: Result<&str, PyErr> = item.extract();
                if let Ok(item) = item {
                    filename = Some(item);
                }
            }
            if let Some(item) = kwargs.get_item("id") {
                let item: Result<&str, PyErr> = item.extract();
                if let Ok(item) = item {
                    id = item //Some(item);
                }
            }
            if let Some(item) = kwargs.get_item("border") {
                let item: Result<bool, PyErr> = item.extract();
                if let Ok(item) = item {
                    border = item;
                }
            }
            if let Some(item) = kwargs.get_item("scale") {
                let item: Result<f64, PyErr> = item.extract();
                if let Ok(item) = item {
                    scale = item;
                }
            }
            if let Some(item) = kwargs.get_item("pages") {
                let item: Result<Vec<usize>, PyErr> = item.extract();
                if let Ok(item) = item {
                    pages = Some(item);
                }
            }
            if let Some(item) = kwargs.get_item("netlist") {
                let item: Result<bool, PyErr> = item.extract();
                if let Ok(item) = item {
                    netlist = item;
                }
            }
            if let Some(item) = kwargs.get_item("theme") {
                let item: Result<&str, PyErr> = item.extract();
                if let Ok(item) = item {
                    theme = item.into();
                }
        ...
        if let Some(filename) = filename {
            ...
            if nb {
                ...
                let mut options = PlotOptions::new(&self.draw.schema, &mut buffer);
                options.id(id) //TODO
                        .border(border)
                        .scale(scale)
                        .netlist(netlist)
                        .theme(theme);
                Plotter::svg(&mut options)?;
                Ok(Some(vec![buffer]))
    ...
    pub fn circuit(&mut self, pathlist: Vec<String>) -> circuit::Circuit {
        let netlist = crate::spice::Netlist::from(&self.draw.schema).unwrap();
