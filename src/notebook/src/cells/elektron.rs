...
use plotter::{gerber, schema::SchemaPlot, svg::SvgPlotter};
...
impl From<&str> for ElektronSubCommand {
    fn from(value: &str) -> Self {
        match value {
            "bom" => ElektronSubCommand::Bom,
            "drc" => ElektronSubCommand::Drc,
            "erc" => ElektronSubCommand::Erc,
            "gerber" => ElektronSubCommand::Gerber,
            "pcb" => ElektronSubCommand::Pcb,
            "schema" => ElektronSubCommand::Schema,
            _ => ElektronSubCommand::NotFound,
        }
    }
}

impl ElektronSubCommand {
    fn execute(
        ...
        if let Some(ArgType::String(command)) = cell.1.get("command") {
            match ElektronSubCommand::from(command.as_str()) {
                ElektronSubCommand::Bom => bom(out, cell, source, dest),
                ElektronSubCommand::Drc => drc(out, cell, source, dest),
                ElektronSubCommand::Erc => erc(out, cell, source, dest),
                ElektronSubCommand::Schema => schema(out, cell, source, dest),
                ElektronSubCommand::Gerber => gerber(out, cell, source, dest),
                ElektronSubCommand::Pcb => pcb(out, cell, source, dest),
                ElektronSubCommand::NotFound =>
...
fn bom(out: &mut dyn std::io::Write,
        cell: &ElektronCell,
        source: &str,
        _dest: &str,
    ) -> Result<(), NotebookError> {
    ...
    for input in input {
        ...
        let doc = SexpParser::load(input_file.to_str().unwrap()).unwrap();
...
fn drc(out: &mut dyn std::io::Write,
        cell: &ElektronCell,
        source: &str,
        _dest: &str,
    ) -> Result<(), NotebookError> {
...
fn erc(out: &mut dyn std::io::Write,
        cell: &ElektronCell,
        source: &str,
        _dest: &str,
    ) -> Result<(), NotebookError> {
    ...
    if let Some(ArgType::List(input)) = args.get("input") {
        ...
        for input in input {
            ...
            let res = erc::erc(Path::new(&input_file)).unwrap();
            count += res.len();
            if !res.is_empty() {
                //output the erc as frontmatter
                writeln!(out, "  {}:", input).unwrap();
...
fn gerber(out: &mut dyn std::io::Write,
        cell: &ElektronCell,
        source: &str,
        dest: &str,
    ) -> Result<(), NotebookError> {
...
fn pcb(out: &mut dyn std::io::Write,
        cell: &ElektronCell,
        source: &str,
        dest: &str,
    ) -> Result<(), NotebookError> {
    ...
    if let Some(ArgType::List(input)) = args.get("input") {
        ...
        for input in input {
            let input_file = Path::new(Path::new(&source).parent().unwrap()) //TODO
                .join(input)
                .join(format!("{}.kicad_pcb", input));
            let output_file = out_dir
                .join(format!("{}_pcb.svg", input));

            let layers = plotter::Pcb::new(&input_file)
            ...
            writeln!(out, "    file: {}", output_file.to_str().unwrap()).unwrap();
...
fn schema(out: &mut dyn std::io::Write,
        cell: &ElektronCell,
        source: &str,
        dest: &str,
    ) -> Result<(), NotebookError> {
...
impl CellWrite<ElektronCell> for CellWriter {
    fn write(
        ...
        ElektronSubCommand::execute(out, cell, source, dest)
    }
}
