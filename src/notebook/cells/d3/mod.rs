...
impl CellWrite<D3Cell> for CellWriter {
    fn write(
        ...
        let y_range: Option<Vec<String>> = if let Some(ArgType::String(y_range)) = args.get("yRange") {
        ...
        let range_data = if let Ok(data) = py_data.extract::<HashMap<String, HashMap<String, Vec<f64>>>>() {
            Some(data)
        } else { None };
        ...
        if let Some(data) = data {
            ...
            return Err(Error::VariableCastError(
        ...
        let colors = COLORS
            .iter()
            .copied()
            .take(if let Some(y_keys) = &y_keys { y_keys.len() } else { 0 } + if let Some(y_range) = &y_range { y_range.len()} else { 0 })
            .collect::<Vec<&str>>();
        ...
        let (min_x, max_x) = if let Some(ArgType::String(xdomain)) = args.get("xDomain") {
            ...
            if tokens.len() == 2 {
                if let (Ok(min), Ok(max)) = (tokens[0].parse::<f64>(), tokens[1].parse::<f64>()) {
                    ...
                    Err(Error::Variable(String::from("Variable xDomain must be \"min, max\"")))
                }
            } else {
                Err(Error::Variable(String::from("Variable xDomain must be \"min, max\"")))
            }
        ...
        let (min_y, max_y) = if let Some(ArgType::String(ydomain)) = args.get("yDomain") {
            ...
            if tokens.len() == 2 {
                if let (Ok(min), Ok(max)) = (tokens[0].parse::<f64>(), tokens[1].parse::<f64>()) {
                    ...
                    Err(Error::Variable(String::from("Variable yDomain must be \"min, max\"")))
                }
            } else {
                Err(Error::Variable(String::from("Variable yDomain must be \"min, max\"")))
            }
...
mod tests {
    ...
    fn test_range() {
        ...
        assert_eq!((-1.3, 1.2), range(None, &[1.0, 1.2, -1.3, 0.9, 0.3, 0.0, -1.0, -0.4]));
