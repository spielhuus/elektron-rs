...
fn main() { // -> Result<(), Error> {
    ...
    match args.command {
        ...
        Command::Bom {
            ...
            match make_bom(&input, group, output, parts) {
                Ok(_) => {},
                Err(err) => {
                    println!("{}", err.to_string()); //TODO
                },
        ...
        Command::Mouser {
            ...
            /* let doc = SexpParser::load(input.as_str()).unwrap();
            let iter = doc.iter();
            let document = SexpItem::from(iter).unwrap();

            let results = bom(&document, true, parts).unwrap();
        ...
        Command::Drc { input, output, } => {
            match make_drc(&input, output) {
                Ok(_) => {},
                Err(err) => println!("{}", err),
            };
        }
        Command::Erc { input, output } => {
            make_erc(&input, output);
        }
        Command::Netlist {
            input,
            output,
